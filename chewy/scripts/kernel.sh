...
curl -s -X POST https://api.telegram.org/bot$BOT_API_KEY/sendMessage -d text="Semaphore CI build for Weeb Kernel MIUI from RaphielCI started ;_;" -d chat_id=@raphiel_ci;
...
if [[ ! -f "${IMAGE}" ]]; then
    ...
    curl -s -X POST https://api.telegram.org/bot$BOT_API_KEY/sendMessage -d text="Semaphore CI build for Weeb Kernel MIUI from RaphielCI stopped unexpectedly, @raphielscape headsup re ;_;" -d chat_id=@raphiel_ci;
    success=false;
    exit 1;
else
